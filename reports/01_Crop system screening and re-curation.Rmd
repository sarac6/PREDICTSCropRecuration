...
By using the PREDICTS curated crop data it came to light that there were two issues that needed attention and re-curation:

#### **1. Dubious mixed crop systems**

Studies were multiple crops are listed in the crop field are not really mixed crop system.
...
(i.e., there are crops where there shouldn't be)

The code utilized by Andy to analyses PREDICTS land use classification and the curated crop data was showing inconsistency between LHU1 and the crop classification (i.e., there are crops where there shouldn't be). Initially he has been ignoring this and the information in the crop field and working just with the predicts classification. But by looking at a study, affecting a large number of sites (31), where LUH1 says Pasture and the crop-curation code says Forage, and actually the PREDICTS classification for these sites was wrong in both studies - the fields are used to grow fodder, rather than being grazed.

#### **3. Predicts site blocks that lack Magpie classes**

Not sure this is a third point or can be merged with one of the above. I will decide when i write it down.
...
-   Re-upload the files into the database
...
(a) look at the other inconsistencies below and, if they need it, re-curate the Excel files such that the sites are recorded as Cropland or Plantation Forest rather than whatever they are at the moment; and

(b) do the same for the two studies I've fixed in code (on the grounds that then they'll be right for future users - but this is obviously much less urgent)? Thanks! The next message here will be the list of sites for each inconsistency; and the following one will be the sites where the Excel file needs changing to Cropland from Pasture. Let me know if anything's not clear.
